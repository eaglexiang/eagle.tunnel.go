...
所有用户信息应该被保存在用户列表中。用户列表是一个文本文件，默认情况下，它的位置为主配置文件同目录下的`users.list`。当这个文件不存在，你需要自行创建。
...
类型|表明当前账户属于独有账户还是共享账户|share/private（二选一）|独有账户（private）
...
```shell
eagle:ppp:100:share
# 用户`eagle`的密码是`ppp`，它的限速是`100KB/s`，不限制同时登录IP
wang:aaa
# 用户`wang`的密码是`aaa`，没有限速，同时只允许一个IP登录  
lan:ccc::share
# 用户`lan`的密码是`ccc`，没有限速，没有登录IP限制
```
